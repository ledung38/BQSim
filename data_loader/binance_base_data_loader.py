...
def resize_data_fields(data_fields, num_days, end_data_di, old_size, new_size):
    for f in data_fields:
        ...
        for d in range(end_data_di):
            for ii in range(old_size): data[d][ii] = old[d][ii]
            for ii in range(old_size, new_size): data[d][ii] = NAN
...
def load_and_process_daily_data(data_fields, data_path, instruments, instrument_map, dates, start_time_secs, di_start, extra_date):
    ...
    for di in range(di_start, num_days):
        ...
        while (ri < len(reads)) and (reads[ri][1] < end_time):
            ...
            for f in data_fields: add_data_value(f[1], f[2], f[3], di, ii, read)
            ri += 1
        # if (di >= d2):
...
def build_from_scratch(data_fields, data_path, data_dates, start_date_index, end_date_index, start_time_secs):
    ...
    for di in range(num_days): dates.append(data_dates[start_date_index + di])
    instruments = []
    instrument_map = {}
    ...
    extra_date = -1
    if (start_time_secs > 0): extra_date = data_dates[end_date_index+1]
    load_and_process_daily_data(data_fields, data_path, instruments, instrument_map, dates, start_time_secs, 0, extra_date)
...
def load_and_update_data(data_dir, data_fields, data_path, data_dates, end_date_index, start_time_secs):
    ...
    num_days = len(dates)
    for f in data_fields: f.append(BDataUtils.load_np_array_data(data_dir, f[0], np.float32, (num_days, num_insts)))
    
    last_date = dates[num_days-1]
    di = bisect.bisect_left(data_dates, last_date)
    if (data_dates[di] == last_date): di += 1
    for i in range(di, end_date_index+1): dates.append(data_dates[i])
    
    if (len(dates) == num_days): return False, dates, instruments, instrument_map
    di_start = num_days
    num_days = len(dates)
    resize_data_fields(data_fields, num_days, di_start, num_insts, num_insts)
    extra_date = -1
    if (start_time_secs > 0): extra_date = data_dates[end_date_index+1]
    load_and_process_daily_data(data_fields, data_path, instruments, instrument_map, dates, start_time_secs, di_start, extra_date)
...
def build_data(xml_node, global_data):
    ...
    data_path = BUtils.format_path(BUtils.get_compulsory_attr(xml_node, "data_path", id))
    ...
    if (start_date_index < back_days): raise Exception("On date " + str(sim_start_date) + ", there are not enough history data for back days " + str(back_days))
    
    if (start_date_index > data_dates_limit): raise Exception("There is no base data on the start date " + str(sim_start_date))

    start_date_index -= back_days

    end_date_index = bisect.bisect_left(data_dates, sim_end_date)
    if (end_date_index > data_dates_limit): end_date_index = data_dates_limit

    if not check_meta_valid(meta_info, data_dates[start_date_index], meta_lines):
        dates, instruments, instrument_map = build_from_scratch(data_fields, data_path, data_dates, start_date_index, end_date_index, start_time_secs)
        random.seed()
        ...
        has_update, dates, instruments, instrument_map = load_and_update_data(data_dir, data_fields, data_path, data_dates, end_date_index, start_time_secs)

    if (has_update):
        BUtils.save_text_file(data_dir + "/dates", dates)
        BUtils.save_text_file(data_dir + "/tickers", instruments)
        for f in data_fields: BDataUtils.save_np_array_data(data_dir, f[0], f[3])
